"""
動画データセットから、ECO用のDataLoaderを作成するために、必要な前処理の関数・クラスを実装
    def make_datapath_list(root_path) # 画像データにしたフォルダへのファイルパスリストを作成
    class VideoTransform()  # 動画を画像にした画像ファイルの前処理クラス、以下の5クラスからなる
        # 画像をまとめてリスケールするクラス、画像の短い方の辺の長さがresizeに変換され、アスペクト比は保たれる
        class GroupResize()
        ...
        # 画像を一つのテンソルにまとめるクラス
        class Stack()
    def get_label_id_dictionary(label_dicitionary_path='../data/kinetics-400/XXXX.csv') # 各動画のラベル名をIDに変換する辞書と、その逆引き辞書を作成
    class VideoDataset(torch.utils.data.Dataset)    # 動画のDataset
"""
# 必要なパッケージのimport
import os
